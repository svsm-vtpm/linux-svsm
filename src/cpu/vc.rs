...
pub fn vc_snp_guest_request(
    ...
    let ghcb: *mut Ghcb = vc_get_ghcb();
    let info1: u64 = pgtable_va_to_pa((*cmd).get_req_shared_page()).as_u64();
    let info2: u64 = pgtable_va_to_pa((*cmd).get_resp_shared_page()).as_u64();
    ...
    unsafe {
        if extended {
            let data_gpa: u64 = pgtable_va_to_pa((*cmd).get_data_gva()).as_u64();
            (*ghcb).set_rax(data_gpa);
            (*ghcb).set_rbx((*cmd).get_data_npages() as u64);
        }
