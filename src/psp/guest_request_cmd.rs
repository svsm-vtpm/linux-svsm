...
use crate::mem::{
    mem_allocate, mem_allocate_frame, pgtable_make_pages_shared, pgtable_pa_to_va, SnpSecrets,
    VMPCK_SIZE,
};
use crate::util::util::memset;
use crate::{
    get_svsm_secrets_page, prints, ALIGN, ALIGNED, BIT, PAGE_COUNT, PAGE_SHIFT, PAGE_SIZE,
};
...
impl SnpGuestRequestMsg {
    ...
    pub fn get_hdr(&self) -> &SnpGuestRequestMsgHdr {
        &self.hdr
    }
}
...
impl SnpGuestRequestCmd {
    ...
    unsafe fn init_ossl_ctx(
        ...
        let __iv_size: c_int = match c_int::try_from(IV_SIZE) {
            Ok(c) => c,
            Err(_) => {
                prints!("ERR: IV_SIZE too big for c_int\n");
                return Err(());
            }
        };
        ret = EVP_CIPHER_CTX_ctrl(ctx, __gcm_set_ivlen, __iv_size, ptr::null_mut());
        if ret != 1 {
    ...
    unsafe fn encrypt_request_unsafe(
        &mut self,
        msg_type: u8,
        plaintext: &[u8],
        plaintext_len: u16,
    ...
    fn encrypt_request(
        ...
        unsafe { self.encrypt_request_unsafe(msg_type, plaintext, plaintext_size) }
    }
    ...
    fn send(&mut self, extended: bool, mut psp_rc: &mut u64) -> Result<(), ()> {
        ...
        match *psp_rc {
            ...
            SNP_GUEST_REQ_INVALID_LEN => {
                if extended {
                    let npages_required: usize = self.get_data_npages();
    ...
    unsafe fn decrypt_response_unsafe(&mut self, msg_type: u8) -> Result<Box<[u8]>, ()> {
        ...
        unsafe {
            if !(*resp).get_hdr().is_valid(msg_type, seqno_last_used()) {
                return Err(());
            }
        }
    ...
    fn decrypt_response(&mut self, msg_type: u8) -> Result<Box<[u8]>, ()> {
        unsafe { self.decrypt_response_unsafe(msg_type) }
    }
